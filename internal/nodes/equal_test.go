...
func Test_Equal(t *testing.T) {
	t.Run("unset first expression", func(t *testing.T) {
		scope := gon.NewScope()
		node := nodes.Equal(nil, nodes.Literal(1))
	...
	t.Run("unset second expression", func(t *testing.T) {
		scope := gon.NewScope()
		node := nodes.Equal(nodes.Literal(1), nil)
	...
	t.Run("cannot compare different types", func(t *testing.T) {
		scope := gon.NewScope()
		node := nodes.Equal(nodes.Literal(1), nodes.Literal(1.))
	...
	t.Run("should return false for inequality", func(t *testing.T) {
		scope := gon.NewScope()
		node := nodes.Equal(nodes.Literal(1), nodes.Literal(2))
...
func Test_Equal_Encoding(t *testing.T) {
	t.Run("should decode with children", func(t *testing.T) {
		require.NotPanics(t, func() {
			node := nodes.Equal(nodes.Literal(true), nodes.Literal(1))
